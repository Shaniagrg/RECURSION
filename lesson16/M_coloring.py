'''
    Given M color There's 4 node 0,1,2,3
    eg: M = 3 
    we have 3 color option 1,2,3
    Steps
        - Start with 0 node then node:1 node:2 and node:3
        - You have to option to start color:1 color:2 or color:3
        - Let's say you start with color:1 check if its adaject to any nodes
        - when you do recursion if you reach all the way from f(0) to f(4) return TRUE
            - WHY? 
            - Cuz all the colors weren't adjacent to any nodes
'''
